...
const useAPI = () => {
  ...
  const getPhotos = async () => {
    try {
      ...
      if (!base) {
        const error = new Error('API base URL is not defined')
        handleApiError(error, 'getPhotos')
        throw error
      }
      ...
      if (!res.ok) {
        const error = new Error(`Failed to fetch photos: ${res.status} ${res.statusText}`)
        handleApiError(error, 'getPhotos')
        throw error
      }
  ...
  const getPhotoImageSettings = async (photoId) => {
    try {
      if (!photoId) throw new Error('photoId is required')

      const res = await fetch(`/api/admin/photos/${encodeURIComponent(photoId)}/image-settings`, { credentials: 'include' })
      ...
      handleApiError(error, 'getPhotoImageSettings')
      throw error
    }
  }

  const updatePhotoImageSettings = async (photoId, settings) => {
    try {
      if (!photoId) throw new Error('photoId is required')

      const res = await fetch(`/api/admin/photos/${encodeURIComponent(photoId)}/image-settings`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        throw new Error(`Failed to update photo image settings: ${res.status} ${res.statusText}`)
      }

      return await res.json()
    } catch (error) {
      handleApiError(error, 'updatePhotoImageSettings')
      throw error
    }
  }
...
export async function getPhotoImageSettings(photoId) {
  if (!photoId) throw new Error('photoId is required')
...
export async function updatePhotoImageSettings(photoId, settings) {
  if (!photoId) throw new Error('photoId is required')
