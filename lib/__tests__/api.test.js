...
describe('useAPI Hook', () => {
  ...
  describe('getPhotoImageSettings', () => {
    ...
    it('should fetch image settings for a photo', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('/api/admin/photos/photo123/image-settings', { credentials: 'include' })
  ...
  describe('updatePhotoImageSettings', () => {
    ...
    it('should update image settings for a photo', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('/api/admin/photos/photo123/image-settings', {
        method: 'PUT',
    ...
    it('should use relative URL even when API base URL is not defined', async () => {
      ...
      let settings
      await act(async () => {
        settings = await result.current.updatePhotoImageSettings('photo123', { brightness: 10 })
      })

      expect(fetch).toHaveBeenCalledWith('/api/admin/photos/photo123/image-settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brightness: 10 }),
        credentials: 'include'
      })
      expect(settings).toEqual(mockUpdatedSettings)
...
describe('Standalone per-photo API exports', () => {
  ...
  describe('updatePhotoImageSettings (standalone)', () => {
    ...
    it('should use relative URL even when base URL missing', async () => {
      ...
      const result = await updatePhotoImageSettings('photo123', { brightness: 10 })
      expect(fetch).toHaveBeenCalledWith('/api/admin/photos/photo123/image-settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brightness: 10 }),
