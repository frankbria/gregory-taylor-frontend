...
describe('ContentContext', () => {
  ...
  describe('per-photo image settings', () => {
    ...
    describe('refreshPhotoSettings', () => {
      ...
      it('should fall back to global defaults on API error', async () => {
        mockPhotoApi.getPhotoImageSettings.mockRejectedValueOnce(new Error('404'))

        const { result } = renderHook(() => useContent(), { wrapper })

        let refreshed
        await act(async () => {
          refreshed = await result.current.refreshPhotoSettings('missing-photo')
        })

        expect(refreshed).toEqual({})
      })

      it('should clear stale cache entry on refresh error', async () => {
        const initialSettings = { quality: 80 }

        mockPhotoApi.getPhotoImageSettings
          .mockResolvedValueOnce(initialSettings)
          .mockRejectedValueOnce(new Error('404'))
          .mockResolvedValueOnce({ quality: 95 })
        ...
        expect(result.current.photoImageSettings['photo1']).toEqual(initialSettings)

        // Refresh fails — should clear the cached entry
        await act(async () => {
          await result.current.refreshPhotoSettings('photo1')
        })
        expect(result.current.photoImageSettings['photo1']).toBeUndefined()

        // Next getPhotoSettings should re-fetch from API, not serve stale cache
        await act(async () => {
          await result.current.getPhotoSettings('photo1')
        })
        expect(mockPhotoApi.getPhotoImageSettings).toHaveBeenCalledTimes(3)
      })
    })
